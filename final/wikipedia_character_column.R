...
#每位來賓佔一個column
guest_list<-as.character(guest_list)

library(dplyr)
library(tidyr)
guest_list<-as.data.frame(guest_list) %>% separate(guest_list, into = paste("guest", 1:30, sep = " "))
View(guest_list)
#write.csv(guest_list, "guest_list.csv") 跑出來是亂碼
