...
#train 有371個column（用來放varialbe），有76020個row（用來放個人資料）
#test  有370個column，有75818個row，比test 少了一column:TARGET
...
#下式的意思是，找出train$TARGET==1的row，在這些row的每一column中找出最小值和最大值
...
#上式中1表示按"row"計算，整段程式碼的意思是數每一row有幾個0，
...
for(pair in features_pair) {
  ...
  if (!(f1 %in% toRemove) & !(f2 %in% toRemove)) {
    if (all(train[[f1]] == train[[f2]])) {
      ...
      #意思是如果f1且f2都不在toRemove 裡，就檢查兩欄是否一樣，如果一樣，就把f2放到toRemove
    }
  }
}
...
train <- train[, feature.names]#刪除掉重複的column
...
for(f in colnames(train)){
  lim <- min(train[,f])
  test[test[,f]<lim,f] <- lim #將test中小於lim 的值，改為lim
  
  lim <- max(train[,f])
  test[test[,f]>lim,f] <- lim  #將test中大於lim 的值，改為lim
}
#---
